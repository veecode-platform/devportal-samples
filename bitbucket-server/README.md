...
Note: there is no ready-to-use preset (`VEECODE_PRESETS`) for Bitbucket, so we configure it manually — this sample mounts an `app-config.local.yaml` and enables the Bitbucket Server dynamic plugins in `dynamic-plugins.yaml`.
